        ...
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          {navItems.map(item => (
            <button key={item.key} onClick={() => nav(item.key)}
              style={{ background: page === item.key ? '#21262d' : 'none', border: 'none', color: page === item.key ? '#e6edf3' : '#8b949e', padding: '6px 14px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: page === item.key ? '600' : '400' }}>
              {item.label}
            </button>
          ))}
          {wallet ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: '8px' }}>
              ...
              <button onClick={disconnectWallet} style={{ background: '#21262d', border: '1px solid #30363d', color: '#8b949e', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}>
                Disconnect
              </button>
            </div>
          ) : (
          ...
          <a href="https://github.com/laney9890/agentbazaar" target="_blank"
            style={{ marginLeft: '4px', background: '#21262d', color: '#e6edf3', border: '1px solid #30363d', padding: '7px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: '600', textDecoration: 'none' }}>
            GitHub
          </a>
        </div>
      </nav>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
          <div style={{ textAlign: 'center', marginBottom: '64px' }}>
            ...
            <h1 style={{ fontSize: '52px', fontWeight: '800', letterSpacing: '-2px', lineHeight: '1.1', marginBottom: '20px' }}>
              The AI Agent<br />
              <span style={{ color: '#238636' }}>Marketplace</span>
            </h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
            {filtered.map(agent => {
              ...
              return (
                <div key={agent.id} onClick={() => goToAgent(agent, 'marketplace')}
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '60px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
            {agents.map(agent => {
              const c = categoryBadge[agent.category] || categoryBadge.Writing
              return (
                <div key={agent.id} onClick={() => goToAgent(agent, 'playground')}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {[
                ...
                <button key={sample.text}
                  onClick={() => { setTask(sample.text); goToAgent(agents[sample.agentIdx], 'playground') }}
          ...
          <div style={{ flex: 1 }}>
            {docsSection === 'how-it-works' && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {[
                    { step: '01', title: 'Browse Agents', desc: 'Explore specialized AI agents across Writing, Development, Analytics and Translation categories.', color: '#238636', action: () => nav('marketplace') },
                    { step: '02', title: 'Describe Your Task', desc: 'Write what you need in plain English. No technical knowledge required.', color: '#1f6feb', action: () => nav('marketplace') },
                    { step: '03', title: 'Pay with USDC', desc: 'Payment is locked in our smart contract on Arc Network. Gasless via Circle Gas Station.', color: '#8957e5', action: () => setDocsSection('payments') },
                    { step: '04', title: 'Agent Completes Work', desc: 'The AI agent processes your task powered by Claude AI. Results delivered instantly.', color: '#f78166', action: () => nav('playground') },
                    { step: '05', title: 'Approve & Release', desc: 'Review the result. If satisfied, approve and payment is released to the agent.', color: '#4ade80', action: () => nav('dashboard') },
                  ].map(item => (
            ...
            {docsSection === 'agents' && (
              <div>
                ...
                {agents.map(agent => {
                  const c = categoryBadge[agent.category] || categoryBadge.Writing
                  return (
                    <div key={agent.id} onClick={() => goToAgent(agent, 'how-it-works')}
            ...
            {docsSection === 'payments' && (
              <div>
                ...
                {[
                  { title: 'USDC Stablecoin', desc: 'Payments are made in USDC, a USD-pegged stablecoin issued by Circle. No price volatility.' },
                  { title: 'Gasless Transactions', desc: 'Arc Network uses Circle Gas Station (ERC-4337). Users pay zero gas fees — all sponsored.' },
                  { title: 'Escrow Protection', desc: 'Funds are locked in our smart contract until you approve the result. No risk of losing money.' },
                  { title: 'Instant Settlement', desc: 'Sub-second finality on Arc Network means payments settle in under 1 second.' },
                ].map(item => (
                  ...
                  <div key={job.id} style={{ background: '#161b22', border: '1px solid #21262d', borderRadius: '12px', padding: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        ...
                        <span style={{ background: '#0d2e1a', color: '#4ade80', border: '1px solid #166534', fontSize: '11px', padding: '2px 10px', borderRadius: '20px' }}>{job.status}</span>
                      </div>
                    </div>
